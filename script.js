...
var insertStatement = "INSERT INTO Contacts (outletname, address, landmaking, address2, sticker,  nivea,  competitor, lga, contact, phone, outlettype, aospw, callfrequency, purchase, currentdate, username, lat, long) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
alert("One or More Fields are empty. Please check and try again.");		
	...
	else
	{
        ...
        db.transaction(function (tx) { tx.executeSql(insertStatement, [outletnametemp, addresstemp, landmaketemp, address2temp, stickertemp,niveatemp, competitortemp, lgatemp, contacttemp, phonetemp, outlettypetemp, aospwtemp, callfrequencytemp, purchasetemp, created_date,user, lat, long ], loadAndReset, onError); });
 ...
 
{
	 ...
	 var retVal = confirm("Please confirm if you have a good internet/data before you continue synchronization.");
	 var counting = 0;
...
success: function(html) {
 ...
 $.ajax({
  ...
  deleteRecord(item['id']);
	...
	alert('Outlet(s) successfully added.. ');
